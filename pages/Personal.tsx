...
import { RotateCw, CheckCircle, Plus, ArrowRightCircle, Trash2, User, Search, ListFilter, Clock, Calendar, CalendarPlus, Zap, ExternalLink, X, Lightbulb, MinusCircle, PlusCircle } from 'lucide-react';
...
export const Personal: React.FC = () => {
  ...
  const [pageTitle, setPageTitle] = useState(() => localStorage.getItem('title_personal') || 'אישי');
  const [pageDesc, setPageDesc] = useState(() => localStorage.getItem('desc_personal') || 'המרחב שלך לצמיחה ואיזון נפשי.');
  const [vision, setVision] = useState(() => localStorage.getItem('vision_personal') || "להיות הגרסה הכי טובה של עצמי...");
  ...
  const [projectData, setProjectData] = useState(() => getSaved('projects_personal', [
    { id: '1', name: 'כושר גופני', why: 'שיפור הבריאות', actions: [{ text: 'ריצה פעמיים בשבוע', range: 'בינוני' }, { text: 'אימון כוח', range: 'בינוני' }] },
  ]));

  const [tasks, setTasks] = useState<Task[]>(() => getSaved('tasks_personal', [
    { id: '1', text: 'משימה אישית ראשונה', completed: false, category: Category.PERSONAL },
  ]));

  const [opportunityData, setOpportunityData] = useState<Opportunity[]>(() => getSaved('opps_personal', [
    { id: 'p1', name: 'קורס מדיטציה', date: '01/05/2026', url: '', category: Category.PERSONAL, status: 'פתוח' },
  ]));

  const [ideas, setIdeas] = useState<{id: string, text: string}[]>(() => getSaved('ideas_personal', []));

  useEffect(() => {
    save('projects_personal', projectData);
    save('tasks_personal', tasks);
    save('opps_personal', opportunityData);
    save('ideas_personal', ideas);
    save('dashboard_focus_headers', projectHeaders);
    localStorage.setItem('title_personal', pageTitle);
    localStorage.setItem('desc_personal', pageDesc);
    localStorage.setItem('vision_personal', vision);
  ...
  const updateActions = (row: any, actionIdx: number, newRange: string) => {
    ...
    if (newRange === 'קצר' && action.text.trim()) {
        ...
        if (!exists) {
            setTasks([{ id: Date.now().toString(), text: action.text, completed: false, category: Category.PERSONAL }, ...tasks]);
        }
    }
  };
  ...
  const renderActions = (row: any) => (
    <div className="space-y-2 py-2 text-right" dir="rtl">
      ...
      <button onClick={() => setProjectData(projectData.map(p => p.id === row.id ? { ...p, actions: [...(p.actions || []), { text: 'פעולה חדשה', range: 'בינוני' }] } : p))} className="text-[10px] font-bold text-indigo-500 hover:text-indigo-700 flex items-center gap-1 mt-1 transition-colors justify-end w-full"><Plus size={10} /> הוסף פעולה</button>
    </div>
  );
  ...
  return (
    <div className="space-y-10 animate-fade-in max-w-7xl mx-auto pb-24 text-right" dir="rtl">
      <div className="flex items-center gap-5 border-b border-slate-200/60 pb-8">
        <div className="w-16 h-16 bg-rose-50 text-rose-500 rounded-2xl flex items-center justify-center shadow-sm ring-4 ring-rose-50/50"><User size={32} /></div>
      ...
      <VisionBoard title="חזון אישי" value={vision} onSave={setVision} id="vision_personal" />

      <SectionContainer id="pers_projects" title="מיקוד אישי" icon={<RotateCw size={20} className="text-indigo-600"/>}>
             <div className="overflow-x-auto">
                 <div className="min-w-[1000px]">
                     <Table data={projectData} columns={[
                            { header: <div className="w-10 text-center">קידום</div>, accessor: (row: any) => (
                                  <div className="flex justify-center">
                                      {row.category !== Category.MAIN ? <button onClick={() => setProjectData(projectData.map(p => p.id === row.id ? {...p, originalCategory: Category.PERSONAL, category: Category.MAIN} : p))} className="p-2 bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white rounded-lg transition-all"><Plus size={16} strokeWidth={3} /></button> : <button onClick={() => setProjectData(projectData.map(p => p.id === row.id ? {...p, category: Category.PERSONAL, originalCategory: undefined} : p))} className="p-2 bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white rounded-lg transition-all"><MinusCircle size={16} strokeWidth={3} /></button>}
                                  </div>
                              ), width: '60px' },
      ...
      <SectionContainer id="pers_tasks" title="משימות אישיות" icon={<CheckCircle size={20} className="text-emerald-600"/>}>
             <div className="divide-y divide-slate-100">
             {activeTasks.length === 0 ? (
                 <div className="py-8 text-center text-slate-300 text-sm italic">אין משימות אישיות פתוחות. בדוק את הארכיון למשימות שהושלמו.</div>
             ) : (
                activeTasks.map(task => (
                    <div key={task.id} className="flex items-center gap-4 group p-4 animate-in fade-in slide-in-from-top-2 duration-300">
                        ...
                        <div className="flex items-center gap-2">
                            <button 
                                onClick={() => setCalendarDraft({ title: task.text, category: 'אישי' })}
             ...
             <button onClick={() => setTasks([{ id: Date.now().toString(), text: '', completed: false, category: Category.PERSONAL }, ...tasks])} className="w-full p-4 text-sm font-bold text-slate-500 hover:text-emerald-600 border-t flex items-center justify-center gap-2"><Plus size={14}/> משימה חדשה</button>
      </SectionContainer>

      <div className="h-96"><CalendarWidget title="יומן אישי" externalDraft={calendarDraft} onModalClose={() => setCalendarDraft(null)} /></div>

      <SectionContainer id="pers_opps" title="הזדמנויות אישיות" icon={<Zap size={20} className="text-amber-500"/>}>
             ...
             <button onClick={() => setOpportunityData([{ id: Date.now().toString(), name: 'הזדמנות חדשה', date: new Date().toLocaleDateString('he-IL'), url: '', category: Category.PERSONAL, status: 'פתוח' }, ...opportunityData])} className="w-full p-4 text-sm font-bold text-slate-500 hover:text-amber-600 border-t flex items-center justify-center gap-2"><Plus size={14}/> הוסף הזדמנות אישית</button>
      </SectionContainer>

      <SectionContainer id="pers_ideas" title="רעיונות אישיים" icon={<Lightbulb size={20} className="text-amber-400"/>}>
        <div className="p-6 space-y-3">
          ...
          <button 
            onClick={() => setIdeas([{id: Date.now().toString(), text: 'רעיון חדש...'}, ...ideas])} 
